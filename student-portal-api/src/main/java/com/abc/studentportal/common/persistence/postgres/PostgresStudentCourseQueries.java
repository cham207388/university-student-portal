...
public class PostgresStudentCourseQueries extends PostgresPageSupport implements StudentCourseQueries {
    ...
    public PostgresStudentCourseQueries(CourseJpaRepository courses, EnrollmentJpaRepository enrollments, PostgresCursorCodec cursors) {
    ...
    public CursorPage<Course> findCoursesByStudent(UUID id, CursorRequest request) {

        return page("courses:student:" + id, request, p -> courses.findDistinctByStudent(id, p), PostgresDomainMapper::course);
    }

    public CursorPage<Student> findStudentsByCourse(UUID id, CursorRequest request) {

        return page("students:course:" + id, request, p -> enrollments.findDistinctStudentsByCourse(id, p), PostgresDomainMapper::student);
    }
