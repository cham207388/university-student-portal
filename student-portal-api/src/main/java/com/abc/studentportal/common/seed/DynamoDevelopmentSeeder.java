...
public class DynamoDevelopmentSeeder implements ApplicationRunner {
    ...
    public void seed() {
        ...
        List<Department> departmentData = List.of(
                new Department(id("department-computing"), "CSE", "Computing and Software Engineering",
                        "Software and systems", CREATED, CREATED, 0),
                new Department(id("department-sciences"), "SCI", "Natural Sciences",
                        "Mathematics and laboratory sciences", CREATED.plusSeconds(1), CREATED.plusSeconds(1), 0),
                new Department(id("department-humanities"), "HUM", "Humanities", "History, writing, and culture",
                        CREATED.plusSeconds(2), CREATED.plusSeconds(2), 0));
        departmentData.forEach(value -> {
            if (departments.findById(value.id()).isEmpty())
                departments.create(value);
        });

        List<Instructor> instructorData = List.of(
                instructor(1, "E1001", "Mira", "Chen", departmentData.get(0).id()),
                instructor(2, "E1002", "Jonah", "Reed", departmentData.get(0).id()),
                instructor(3, "E1003", "Priya", "Nair", departmentData.get(1).id()),
                instructor(4, "E1004", "Mateo", "Silva", departmentData.get(1).id()),
                instructor(5, "E1005", "Elena", "Brooks", departmentData.get(2).id()));
        instructorData.forEach(value -> {
            if (instructors.findById(value.id()).isEmpty())
                instructors.create(value);
        });

        String[][] names = {{"Avery", "Morgan"}, {"Noah", "Williams"}, {"Sofia", "Patel"}, {"Liam", "Kim"},
                {"Zoe", "Martinez"},
                {"Ethan", "Nguyen"}, {"Maya", "Johnson"}, {"Lucas", "Garcia"}, {"Amara", "Wilson"},
                {"Owen", "Davis"}};
        ...
        for (int index = 0; index < names.length; index++) {
            ...
            Student value = new Student(id("student-" + (index + 1)), "S" + String.format("%04d", index + 1),
            ...
            if (students.findById(value.id()).isEmpty())
                students.create(value);
            if (profiles.findByStudentId(value.id()).isEmpty())
                profiles.create(profile(value, index));
        }

        CourseStatus[] targets = {CourseStatus.OPEN, CourseStatus.OPEN, CourseStatus.DRAFT, CourseStatus.CLOSED,
                CourseStatus.COMPLETED, CourseStatus.OPEN, CourseStatus.CANCELLED, CourseStatus.OPEN,
                CourseStatus.DRAFT, CourseStatus.OPEN};
        ...
        for (int index = 0; index < 10; index++) {
            ...
            CourseStatus initialStatus = targets[index] == CourseStatus.DRAFT
                    || targets[index] == CourseStatus.CANCELLED
                    ? targets[index]
                    : CourseStatus.OPEN;
            Course value = new Course(id("course-" + (index + 1)), "CSE-" + (101 + index),
                    "University Course " + (index + 1),
                    "Fictional development course", 3 + index % 2, index == 0 ? 2 : 20, CourseStatus.OPEN,
                    instructor.departmentId(), instructor.id(), CREATED.plusSeconds(50 + index),
                    CREATED.plusSeconds(50 + index), 0);
            value = new Course(value.id(), value.courseCode(), value.title(), value.description(), value.credits(),
                    value.capacity(),
                    initialStatus, value.departmentId(), value.instructorId(), value.createdAt(), value.updatedAt(),
                    value.version());
            courseData.add(value);
            if (courses.findById(value.id()).isEmpty())
        ...
        for (int index = 0; index < targets.length; index++)
            setCourseStatus(courseData.get(index).id(), targets[index]);
        ...
        LOGGER.info(
                "event=development_seed_completed departments=3 students=10 profiles=10 instructors=5 courses=10 enrollments=6");
    }
    ...
    private void ensureEnrollment(int number, Student student, Course course, EnrollmentStatus target) {
        ...
        if (current == null) {
            ...
            EnrollmentStatus initial = target == EnrollmentStatus.WAITLISTED ? EnrollmentStatus.WAITLISTED
                    : EnrollmentStatus.ENROLLED;
            current = enrollments.create(
                    new Enrollment(enrollmentId, student.id(), course.id(), initial, time, null, null, time, time, 0));
        }
    ...
    private void setCourseStatus(UUID id, CourseStatus status) {
        ...
        courses.update(new Course(changed.id(), changed.courseCode(), changed.title(), changed.description(),
                changed.credits(),
                changed.capacity(), changed.status(), changed.departmentId(), changed.instructorId(),
                changed.createdAt(), changed.updatedAt(), current.version()));
    }
    ...
    private void setStudentStatus(UUID id, StudentStatus status) {
        ...
        students.update(new Student(changed.id(), changed.studentNumber(), changed.firstName(), changed.lastName(),
                changed.email(),
