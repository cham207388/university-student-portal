...
public final class StudentDynamoMapper {
	...
	public static StudentDynamoRecord toRecord(Student value) {
		...
		record.setId(value.id().toString()); record.setStudentNumber(value.studentNumber());
		record.setFirstName(value.firstName()); record.setLastName(value.lastName()); record.setEmail(value.email());
		record.setStatus(value.status().name()); record.setDepartmentId(value.departmentId().toString());
		record.setEntityType("STUDENT"); record.setCreatedAt(value.createdAt().toString());
	...
	public static Student toDomain(StudentDynamoRecord value) {
		return new Student(UUID.fromString(value.getId()), value.getStudentNumber(), value.getFirstName(), value.getLastName(),
