...
public final class StudentProfileDynamoMapper {
	...
	public static StudentProfileDynamoRecord toRecord(StudentProfile value) {
		...
		record.setDateOfBirth(value.dateOfBirth().toString());
		record.setPhoneNumber(value.phoneNumber());
		record.setAddressLine1(value.addressLine1());
		record.setAddressLine2(value.addressLine2());
		record.setCity(value.city());
		record.setState(value.state());
		record.setPostalCode(value.postalCode());
		record.setCountry(value.country());
	...
	public static StudentProfile toDomain(StudentProfileDynamoRecord value) {
		return new StudentProfile(UUID.fromString(value.getId()), UUID.fromString(value.getStudentId()),
				...
				value.getCity(), value.getState(), value.getPostalCode(), value.getCountry(),
				Instant.parse(value.getCreatedAt()),
