...
public class StudentController {
	...
	@PostMapping ResponseEntity<StudentApi.Response> create(@Valid @RequestBody StudentApi.CreateRequest request) {
		...
		return ResponseEntity.created(URI.create("/api/v1/students/" + value.id())).body(StudentMapper.toResponse(value));
	}
	@GetMapping("/{id}") StudentApi.Response get(@PathVariable UUID id) { return StudentMapper.toResponse(service.get(id)); }
	@GetMapping CursorPageResponse<StudentApi.Response> list(@RequestParam(required = false) UUID departmentId,
		...
		if (departmentId != null && status != null) throw new InvalidRequestException("DynamoDB student lists support one filter at a time");
		if (lastName != null && departmentId == null) throw new InvalidRequestException("lastName requires departmentId in DynamoDB mode");
	...
	@PutMapping("/{id}") StudentApi.Response update(@PathVariable UUID id, @Valid @RequestBody StudentApi.UpdateRequest request) {
		return StudentMapper.toResponse(service.update(id, new StudentService.UpdateCommand(request.studentNumber(), request.firstName(),
				request.lastName(), request.email(), request.departmentId(), request.version())));
	}
	@PatchMapping("/{id}/status") StudentApi.Response status(@PathVariable UUID id,
	...
	@PutMapping("/{id}/profile") StudentApi.ProfileResponse putProfile(@PathVariable UUID id,
	...
	@GetMapping("/{id}/profile") StudentApi.ProfileResponse profile(@PathVariable UUID id) {
		return StudentMapper.toResponse(service.getProfile(id));
	}
	@DeleteMapping("/{id}/profile") ResponseEntity<Void> deleteProfile(@PathVariable UUID id, @RequestParam long version) {
		service.deleteProfile(id, version); return ResponseEntity.noContent().build();
	}
