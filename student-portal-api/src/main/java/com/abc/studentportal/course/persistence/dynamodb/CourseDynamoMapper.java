...
public final class CourseDynamoMapper {
    ...
    public static CourseDynamoRecord toRecord(Course value) {
        ...
        record.setTitle(value.title());
        record.setDescription(value.description());
        record.setCredits(value.credits());
        record.setCapacity(value.capacity());
        record.setOccupiedSeats(0L);
        record.setEnrollmentCount(0L);
        record.setStatus(value.status().name());
        record.setDepartmentId(value.departmentId().toString());
        record.setInstructorId(value.instructorId().toString());
        record.setEntityType("COURSE");
        ...
        record.setCourseCodeId(DynamoSortKeys.textId(value.courseCode(), value.id()));
