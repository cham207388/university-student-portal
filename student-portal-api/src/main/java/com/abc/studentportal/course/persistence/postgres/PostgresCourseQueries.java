...
public class PostgresCourseQueries extends PostgresPageSupport implements CourseQueries {
    ...
    public PostgresCourseQueries(CourseJpaRepository repository, PostgresCursorCodec cursors) {
    ...
    public CursorPage<Course> findAll(CursorRequest request) {
        ...
        return page("courses", request, repository::findAll, PostgresDomainMapper::course);
    }

    public CursorPage<Course> findByDepartment(UUID id, CursorRequest request) {

        return page("courses:department:" + id, request, p -> repository.findByDepartment_Id(id, p), PostgresDomainMapper::course);
    }

    public CursorPage<Course> findByInstructor(UUID id, CursorRequest request) {

        return page("courses:instructor:" + id, request, p -> repository.findByInstructor_Id(id, p), PostgresDomainMapper::course);
    }

    public CursorPage<Course> findByStatus(CourseStatus status, CursorRequest request) {

        return page("courses:status:" + status, request, p -> repository.findByStatus(status, p), PostgresDomainMapper::course);
    }
