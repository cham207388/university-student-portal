...
public final class DynamoEnrollmentTransactionWriter {
	...
	public DynamoEnrollmentTransactionWriter(DynamoDbClient client, DynamoDbTables tables) {
		this.client = client; this.tables = tables;
	}
	...
	public Enrollment update(Enrollment previous, Enrollment next) {
		...
		if (capacityDelta != 0) actions.add(changeCapacity(next.courseId().toString(), capacityDelta));
		actions.add(putEnrollment(record, "#version = :version", Map.of(":version", number(next.version()))));
		if (previous.isActive() && !next.isActive()) actions.add(deleteActiveLock(previous));
	...
	private TransactWriteItem addStudentEnrollment(Enrollment value) {
		return TransactWriteItem.builder().update(Update.builder()
				...
				.updateExpression("SET enrollmentCount = if_not_exists(enrollmentCount, :zero) + :one, #version = #version + :one")
	...
	private TransactWriteItem addCourseEnrollment(String courseId, boolean consumesCapacity) {
		...
		Map<String, String> names = consumesCapacity
				? Map.of("#status", "status", "#occupied", "occupiedSeats", "#capacity", "capacity", "#version", "version")
				: Map.of("#status", "status", "#version", "version");
		return TransactWriteItem.builder().update(Update.builder().tableName(tables.courses().tableName()).key(key(courseId))
				.updateExpression(update).conditionExpression(condition).expressionAttributeNames(names)
				.expressionAttributeValues(Map.of(":open", string("OPEN"), ":zero", number(0), ":one", number(1)))
				.build()).build();
	}
	...
	private TransactWriteItem changeCapacity(String courseId, int delta) {
		...
		Map<String, String> names = delta > 0
				? Map.of("#status", "status", "#version", "version", "#occupied", "occupiedSeats", "#capacity", "capacity")
	...
	private TransactWriteItem putEnrollment(EnrollmentDynamoRecord record, String condition,
		...
		if (condition.contains("#version")) put.expressionAttributeNames(Map.of("#version", "version"));
		if (!values.isEmpty()) put.expressionAttributeValues(values);
	...
	private void execute(List<TransactWriteItem> actions, String message) {
		try { client.transactWriteItems(request -> request.transactItems(actions)); }
		catch (TransactionCanceledException exception) { throw new ConflictException(message); }
	}

	private static String lockId(Enrollment value) { return "ACTIVE#" + value.studentId() + "#" + value.courseId(); }
	private static Map<String, AttributeValue> key(String id) { return Map.of("id", string(id)); }
