...
public class PostgresEnrollmentService extends EnrollmentService {
    ...
    public Enrollment enroll(UUID studentId, UUID courseId) {
        ...
        if (student.getStatus() != StudentStatus.ACTIVE) throw new ConflictException("Student is not active and cannot enroll");
    ...
    public Enrollment changeStatus(UUID id, EnrollmentStatus target, String finalGrade, long version) {
        ...
        EnrollmentEntity current = enrollmentEntities.findLockedById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Enrollment", id));
        if (current.getVersion() != version) throw new ObjectOptimisticLockingFailureException(EnrollmentEntity.class, id);
    ...
    @Override
    public Enrollment drop(UUID id, long version) { return changeStatus(id, EnrollmentStatus.DROPPED, null, version); }
