...
public class EnrollmentController {
	private final EnrollmentService service; private final DynamoEnrollmentQueries queries;
	public EnrollmentController(EnrollmentService service, DynamoEnrollmentQueries queries) { this.service = service; this.queries = queries; }
	@PostMapping ResponseEntity<EnrollmentApi.Response> create(@Valid @RequestBody EnrollmentApi.CreateRequest request) {
		var value = service.enroll(request.studentId(), request.courseId());
		return ResponseEntity.created(URI.create("/api/v1/enrollments/" + value.id())).body(EnrollmentMapper.toResponse(value));
	}
	@GetMapping("/{id}") EnrollmentApi.Response get(@PathVariable UUID id) { return EnrollmentMapper.toResponse(service.get(id)); }
	@GetMapping CursorPageResponse<EnrollmentApi.Response> list(@RequestParam(required = false) UUID studentId,
		...
		if (filters > 1) throw new InvalidRequestException("DynamoDB enrollment lists support one relationship or status filter at a time");
	...
	@PatchMapping("/{id}/status") EnrollmentApi.Response status(@PathVariable UUID id,
			@Valid @RequestBody EnrollmentApi.StatusRequest request) {
		return EnrollmentMapper.toResponse(service.changeStatus(id, request.status(), request.finalGrade(), request.version()));
	}
	@DeleteMapping("/{id}") EnrollmentApi.Response drop(@PathVariable UUID id, @RequestParam long version) {
